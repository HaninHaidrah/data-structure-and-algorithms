...
const toLastNames = people => {
  // Solution code here...
  const firstAndLatName=people.map((el)=>{
    return `${el.firstName} ${el.lastName}`
  })
  return firstAndLatName;
};
...
const addValues = (arr) => {
  // Solution code here...
 let sum =arr.reduce((accumaltor,el)=>{
     accumaltor+=el
     return accumaltor;
  },0)
return sum;
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let sum=[] 
  arr.reduce((accumaltor,el)=>{
    accumaltor+=el
sum.push(accumaltor)  },0)
  return (sum.length);
};
