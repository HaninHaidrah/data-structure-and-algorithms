...
const toTitleCase = (arr) => {
  let result = [];
  arr.map((element) => {
    return result.push(element.charAt(0).toUpperCase() + element.slice(1));
  });
  return result;
};
...
const sortBy = (property, arr) => {
  return arr.sort((now, last) => (now[property] > last[property] ? 1 : -1));
};
...
const detectTicTacToeWin = (board) => {
  const win = (board) => {
    for (let i = 0; i < board.length; i++) {
      const x = board[i].every((row) => row === "X");
      const y = board[i].every((row) => row === "O");
      if (x || y) return true;
      else return false;
    }
  };
  const revers = board.map((row, i, arr) => {
    return row.map((_, j) => arr[j][i]);
  });
  const diagonal = board.map((_, i, arr) => {
